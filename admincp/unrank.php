...
*/
require("monit.php");
echo "<title>$config->title Search And Unrank Users</title>";
...
include"topnav.php";
echo"<div class='breadcrumb'><a href='../member/index.php'>Home</a> &raquo; Search And Delete users</div><br/>";
echo"<center>
...
echo"<ul><li>[$i] $st$username<br/><a href='?action=Delete&yes=true&id=$id'>Unrank</a></li></ul>";
...
{ $id=(int)$_GET["id"];
$query=mysql_query("UPDATE b_users SET level='0' WHERE userID=$id");
if($query)
{
$msg="User Unranked Successfuly";
}
else
{
$msg=mysql_error();
...
$id=(int)$_GET["id"];
echo"<div class='topnav'>System Warning</div><div class='msg'>Are You Sure You want to Delete This User from Guruszanga Database ?</div><div class='gap'></div><div class='button'><a href='?action=Delete&yes=true&id=$id'>
...
$rank=getrank($level);
mysql_query("UPDATE b_users SET level='0' WHERE userID=$id") or die(mysql_error());
...
echo"<div class='link_button'><a href='search.php'>CLICK TO SEARCH AGAIN </a></div>";
include"footer.php";
?>
