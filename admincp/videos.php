...
{ $id=(int)$_GET["id"];
$query=mysql_query("DELETE FROM b_videocat WHERE id=$id");
if($query)
{
...
$id=(int)$_GET["id"];
echo"<div class='topnav'>System Warning</div><div class='msg'>Are You Sure You want to Delete This Category ?</div><div class='gap'></div><div class='button'><a href='?action=Delete&yes=true&id=$id'><font color='red'>Yes</font></a> | <div class='right'><a href='videos.php'>No</a></div></div>";
}
elseif($action=="Edit")
{
$id=(int)$_GET["id"];
...
{
mysql_query("UPDATE b_videocat SET name='$name' WHERE id=$id") or die(mysql_error());
...
mysql_query("UPDATE b_videocat SET name='$name', img='$img' WHERE id=$id");
...
$query=mysql_query("SELECT * FROM b_videocat WHERE id='$id'");
...
echo"<div class='button'><a href='videos.php'>Back</a></div>";
exit();
}
elseif($action=="Add")
{
...
{
$name=cleanvalues2($_POST["name"]);
$checkname=mysql_num_rows(mysql_query("SELECT * FROM b_videocat WHERE name='$name'"));
if(empty($name) || strlen($name)<4)
{
echo"<div class='center'><font color='red'>Category field name is empty or less than 4</font></div>";
}
...
if(empty($_FILES["file"]) || $_FILES["file"]["size"]==0)
{
mysql_query("INSERT INTO b_videocat SET name='$name'") or die(mysql_error());
...
{
move_uploaded_file($_FILES["file"]["tmp_name"], $folder.$img);
mysql_query("INSERT INTO b_videocat SET name='$name', img='$img' WHERE id=$id");
...
{
echo"<div class='topnav'><div class='left'><a href='videos.php?action=Add'>Create</a></div><div class='right'><a href='vfiles.php'>Uploaded videos</a></div></div><div class='gap'></div><div class='center'><ul>Here You can Create New Categories, delete or edit existing category categories</ul></div>";
$msg=$_GET["msg"];
if(!empty($msg))
{
echo"<div class='msg'>$msg</div>";
}
$query=mysql_query("SELECT * FROM b_videocat ORDER BY id DESC") or mysql_error();
$count=mysql_num_rows($query);
echo"<div class='topnav'><b>Video Categories</b>($count)</div>";
if($count>0)
{
