...
{ $id=(int)$_GET["id"];
$query=mysql_query("DELETE FROM b_filmcat WHERE id=$id");
if($query)
{
...
{
mysql_query("UPDATE b_filmcat SET name='$name' WHERE id=$id") or die(mysql_error());
...
mysql_query("UPDATE b_filmcat SET name='$name', img='$img' WHERE id=$id");
...
$query=mysql_query("SELECT * FROM b_filmcat WHERE id='$id'");
...
echo"<div class='link_button'><a href='?'>Back</a></div>";
exit();
}
elseif($action=="Add")
{
...
{
$name=cleanvalues2($_POST["name"]);
$checkname=mysql_num_rows(mysql_query("SELECT * FROM b_filmcat WHERE name='$name'"));
if(empty($name) || strlen($name)<4)
{
echo"<div class='center'><font color='red'>Category field name is empty or less than 4</font></div>";
}
...
if(empty($_FILES["file"]) || $_FILES["file"]["size"]==0)
{
mysql_query("INSERT INTO b_filmcat SET name='$name'") or die(mysql_error());
...
{
move_uploaded_file($_FILES["file"]["tmp_name"], $folder.$img);
mysql_query("INSERT INTO b_filmcat SET name='$name', img='$img' WHERE id=$id");
...
{
echo"<div class='topnav'><div class='left'><a href='?action=Add'>Create</a></div><div class='right'><a href='ffiles.php'>Uploaded Medias</a></div></div><div class='gap'></div><div class='center'><ul>Here You can Create New Categories, delete or edit existing media categories</ul></div>";
$msg=$_GET["msg"];
if(!empty($msg))
{
echo"<div class='msg'>$msg</div>";
}
$query=mysql_query("SELECT * FROM b_filmcat ORDER BY id DESC") or mysql_error();
$count=mysql_num_rows($query);
echo"<div class='topnav'><b>Media Categories</b>($count)</div>";
if($count>0)
{
...
echo"<div class='link_button'><a href='films'>Back</a></div>";
include"../footer.php";
}
?>
