...
*/require("monit.php");
echo"<div class='topnav'><a href='jamz.php'>Music Menu</a></div><div class='center'>Here You can move/delete uploaded music</div>";
$msg=$_GET["msg"];
if(!empty($msg))
{
...
$id=(int)$_POST["id"];
$cid=(int)$_POST["cid"];
mysql_query("UPDATE b_jamz SET catid='$cid' WHERE id=$id");
$cinfo=mysql_fetch_assoc(mysql_query("SELECT * FROM b_jamzcat WHERE id=$cid"));
$cname=$cinfo["name"];
...
$query=mysql_query("SELECT * FROM b_jamzcat");
...
{ $id=(int)$_GET["id"];
$query=mysql_query("DELETE FROM b_jamz WHERE id=$id");
if($query)
{
...
$id=(int)$_GET["id"];
echo"<div class='topnav'>System Warning</div><div class='msg'>Are You Sure You want to Delete This Music ?</div><div class='gap'></div><div class='button'><a href='?action=Delete&yes=true&id=$id'><font color='red'>Yes</font></a> | <div class='right'><a href='?'>No</a></div></div>";
exit();
}
...
$numrows=mysql_num_rows(mysql_query("SELECT * FROM b_jamz"));
...
$tquery=mysql_query("SELECT * FROM b_jamz ORDER BY id DESC LIMIT $offset, $rowsperpage");
...
{
echo"<div class='title'>Uploaded Music</div>";
...
$catid=$info["catid"];
$cinfo=mysql_fetch_assoc(mysql_query("SELECT * FROM b_jamzcat WHERE id=$catid"));
$cat=$cinfo["name"];
