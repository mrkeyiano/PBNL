...
$cquery=mysql_query("SELECT * FROM b_moviescat WHERE id=$id");
...
$name=$cinfo["name"];
echo"<title>$config->title- $name Videos</title>";
...
include"../topnav.php";
echo"<div class='breadcrumb'><a href='index.php'>Videos Home</a></div>";
echo"<center>";
include"../ads.php";
fblike();
...
<div class='b_head'>$name Videos</div>";
...
$numrows=mysql_num_rows(mysql_query("SELECT * FROM b_movies WHERE catid=$id"));
...
if($sort==1)
{
$tquery=mysql_query("SELECT * FROM b_movies WHERE catid=$id ORDER BY id DESC LIMIT $offset, $rowsperpage");
}
elseif($sort==2)
{
$tquery=mysql_query("SELECT * FROM b_movies WHERE catid=$id ORDER BY id ASC LIMIT $offset, $rowsperpage");
}
elseif($sort==3)
{
$tquery=mysql_query("SELECT * FROM b_movies WHERE catid=$id ORDER BY downloads LIMIT $offset, $rowsperpage");
}
elseif($sort==4)
{
$tquery=mysql_query("SELECT * FROM b_movies WHERE catid=$id ORDER BY `views` LIMIT $offset, $rowsperpage");
}
else
{
$tquery=mysql_query("SELECT * FROM b_movies WHERE catid=$id ORDER BY id DESC LIMIT $offset, $rowsperpage");
