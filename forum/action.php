...
echo"<div class='msg'><font color='red'>*clap* Is that hacking? wow so cool buh fortunate, You dont have this permission</font></div>";
include"../footer.php";
exit();
}
include"../topnav.php";
...
{
echo"<div class='msg'><font color='red'>invalid forum</font></div>";
}
else
{
$query=mysql_query("UPDATE b_topics SET locked=1 WHERE id=$id");
...
$id=$_POST["id"];
$message=cleanvalues($_POST["message"]); $subject=cleanvalues($_POST["subject"]); if(empty($message) || strlen($message)<4 || empty($subject) || strlen($subject)<4) { echo"<div class='msg'>Please make sure you entered message title and message and it must be greater than four characters</div>"; } else { mysql_query("UPDATE b_topics SET `subject`='$subject', `message`='$message' WHERE id=$id") or die(mysql_error()); $msg="Saved successfuly"; header("location: showtopic.php?id=$id&amsg=$msg"); exit(); } }
$id=(int)$_GET["tid"];
$info=mysql_fetch_assoc(mysql_query("SELECT * FROM b_topics WHERE id=$id"));
$message=cleanvalues2($info["message"]); $subject=cleanvalues2($info["subject"]); echo"<form action='?action=edittopic' method='POST'><ul><li>Topic Title<br/><input type='text' name='subject' value='$subject'></li><input type='hidden' name='id' value='$id'><li>Message<br/><textarea name='message'>$message</textarea></li><li><center><input type='submit' name='submit' value='Save Changes' class='button'></center></li></ul></form>";
exit();
}
...
$name=$finfo["name"];
$tid=(int)$_POST["tid"];
mysql_query("UPDATE b_topics SET forumid='$id' WHERE id=$tid") or die(mysql_error());
...
{
$query=mysql_query("UPDATE b_replies SET message='$message' WHERE id=$id");
...
{
$query=mysql_query("UPDATE b_topics SET locked=0 WHERE id=$id");
